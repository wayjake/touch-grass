...
void generateRiver() {
    ...
    if (horizontal) {
        int y = random(1, MAP_HEIGHT - 1);
        for (int x = 0; x < MAP_WIDTH; x++) {
            tg_map[y][x] = TG_WATER;
            int drift = random(3) - 1;
            y += drift;
            if (y < 1) y = 1;
            if (y >= MAP_HEIGHT - 1) y = MAP_HEIGHT - 2;
        }
    } else {
        int x = random(1, MAP_WIDTH - 1);
    ...
    for (int dy = -size; dy <= size; dy++) {
        for (int dx = -size; dx <= size; dx++) {
            ...
            if (x >= 0 && x < MAP_WIDTH && y >= 0 && y < MAP_HEIGHT) {
                if (tg_map[y][x] == TG_GRASS && random(100) < 60) {
                    tg_map[y][x] = TG_TREE;
                }
            }
        }
...
void generateTerrain() {
    ...
    int numRivers = random(1, 3);
    for (int i = 0; i < numRivers; i++) {
        generateRiver();
    }

    // Step 3: Generate 2-4 tree clusters
    int numClusters = random(2, 5);
    for (int i = 0; i < numClusters; i++) {
        int cx = random(2, MAP_WIDTH - 2);
        int cy = random(1, MAP_HEIGHT - 1);
        generateTreeCluster(cx, cy, random(1, 3));
    }

    // Step 4: Scatter dirt patches (~10% of remaining grass)
    for (int y = 0; y < MAP_HEIGHT; y++) {
        for (int x = 0; x < MAP_WIDTH; x++) {
            if (tg_map[y][x] == TG_GRASS && random(100) < 10) {
                tg_map[y][x] = TG_DIRT;
            }
        }
    }
