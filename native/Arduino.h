/*
 * Arduino.h shim for native (Mac/Linux) build
 *
...
static inline long random_1(long max) {
    return rand() % max;
}

// Arduino random(min, max) - returns min to max-1
static inline long random_2(long min, long max) {
    return min + (rand() % (max - min));
}

// Overload trick - use macro to select version by arity
#define GET_RANDOM_MACRO(_1, _2, NAME, ...) NAME
#define random(...) GET_RANDOM_MACRO(__VA_ARGS__, random_2, random_1)(__VA_ARGS__)
