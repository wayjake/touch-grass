...
uint8_t findPredictions(const char* prefix, const char** results, uint8_t maxResults) {
    ...
    uint16_t startOffset = T9_DICT_INDEX[letterIdx];
    uint16_t endOffset = T9_DICT_INDEX[letterIdx + 1];
...
void copyDictWord(const char* dictPtr, char* buffer, uint8_t maxLen) {
    uint8_t i = 0;
    char c;
    while ((c = dictPtr[i]) != '\0' && i < maxLen - 1) {
        buffer[i++] = c;
    }
    buffer[i] = '\0';
}
