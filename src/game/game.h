 ...
 *
 * Game state machine: all game logic, rendering, and input handling.
 */
...
// Melodies
static const int startupMelody[] = { NOTE_G4, NOTE_C5, NOTE_E5, NOTE_G5 };
static const int startupDurations[] = { 100, 100, 100, 300 };
#define STARTUP_LEN 4

static const int deathMelody[] = { NOTE_E4, NOTE_D4, NOTE_C4, NOTE_REST };
static const int deathDurations[] = { 200, 200, 400, 100 };
#define DEATH_LEN 4
...
static bool executeItemAction(ItemType item, uint8_t action) {
    ...
    if (item == ITEM_COOKED_FRUIT && strcmp(actionName, "Eat") == 0) {
        if (removeItem(ITEM_COOKED_FRUIT, 1)) {
    ...
    if (item == ITEM_SEED && strcmp(actionName, "Plant") == 0) {
        if (tg_underPlayer == TG_DIRT && removeItem(ITEM_SEED, 1)) {
            setTileUnderPlayer(TG_SEEDLING);
            platform_beep(NOTE_C5, 50);
            platform_beep(NOTE_E5, 50);
            processAction();
    ...
    if (item == ITEM_DIRT && strcmp(actionName, "Place") == 0) {
        if (tg_underPlayer == TG_WATER && removeItem(ITEM_DIRT, 1)) {
            setTileUnderPlayer(TG_DIRT);
            platform_beep(NOTE_G4, 50);
            platform_beep(NOTE_C5, 50);
            processAction();
...
static void drawTile16(int x, int y, const uint8_t* sprite) {
    ...
    for (int row = 0; row < 16; row++) {
        uint8_t byte1 = sprite[row * 2];
        uint8_t byte2 = sprite[row * 2 + 1];
...
static void drawInventoryScreen() {
    ...
    if (filledSlots == 0) {
        ...
        int menuY = 14;
        uint8_t displayCount = (filledSlots < 4 ? filledSlots : 4);
    ...
    if (gameState == STATE_SPLASH) {
        ...
        if (now - chestAnimTime >= CHEST_ANIM_DELAY) {
            ...
            if (chestAnimFrame == 1) {
                ...
                addItem(ITEM_SEED, 3);
                addItem(ITEM_HAMMER, 1);
                addItem(ITEM_AXE, 1);
                addItem(ITEM_LASSO, 1);
                setTileUnderPlayer(TG_DIRT);
                processAction();
