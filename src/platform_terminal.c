/*
 * Terminal Platform Implementation (macOS / Linux)
 *
 * Implements the platform API (platform.h) as a raw POSIX executable.
...
void platform_draw_sprite_scaled(int x, int y, const uint8_t* spriteData, int scale) {
    for (int row = 0; row < 8; row++) {
        uint8_t rowData = spriteData[row];
...
void platform_draw_xbm(int x, int y, int w, int h, const uint8_t* bitmap, bool color) {
    ...
    for (int j = 0; j < h; j++) {
        for (int i = 0; i < w; i++) {
            uint8_t byteVal = bitmap[j * byteWidth + i / 8];
